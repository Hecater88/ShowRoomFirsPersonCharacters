...
public class FPSControllerOnline : NetworkBehaviour {
	...
	//para controlar cuando el jugador se encuentra dentro del menu
	public bool inMenu;
	...
	void Start () {
		...
		// si el jugador es el local, le permitiremos realizar las interacciones
		if (isLocalPlayer) {
			canShoot = true;
		}
	}
	...
	void Update () {
		...
		//si el jugador está en menú, salimos del método para evitar que pueda realizar acciones
		if (inMenu) {
			return;
		}
	...
	/// </summary>
	void Movement (){
		//si el jugador está en el menu, salgo del metodo para evitar que realice movimientos
		if(inMenu){
			return;
		}
	...
	// los clientes pedirán mediante comandos, que se ejecuten acciones en el servidor
	[Command]
	/// <summary>
	...
	// esto hace que el metodo permita ser invocado en los clientes, desde el servidor
	[ClientRpc]
	/// <summary>
	...
	[Command]
	/// <summary>
	/// Realiza la petición de recarga al servidor
	/// </summary>
	void CmdReload () {
		RpcReload ();
	}
	...
	[ClientRpc]
	/// <summary>
	/// Realiza la petición de recarga en todas las intancias de los clientes
	/// </summary>
	void RpcReload () {
